...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        values['directory_count'] = request.env['document.directory'].sudo().search_count([])
        return values
...
#    def content_common(self, xmlid=None, model='ir.attachment', id=None, field='datas',
    def document_content_common(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        return request.env['ir.binary']._get_stream_from(attachement_id).get_response(as_attachment=download)
        if status != 200:
